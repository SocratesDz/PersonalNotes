...
final public class GoogleDriveActions {
    ...
    private static com.google.api.services.drive.Drive mGoogleService;

    static void init(NoteDetailActivity context, String email) {
        if (context != null && email != null) {
            ...
            mGoogleService = new com.google.api.services.drive.Drive.Builder(
    ...
    static void init(NotesActivity context, String email) {
        if (context != null && email != null) {
            mGAC = new GoogleApiClient.Builder(context).addApi(Drive.API)
                    .addScope(Drive.SCOPE_FILE).setAccountName(email)
                    .addConnectionCallbacks(context).addOnConnectionFailedListener(context).build();
    ...
    static ArrayList<GF> search(DriveId prId, String titl, String mime) {
        ...
        if (isConnected()) {
            ...
            if (prId != null) filters.add(Filters.in(SearchableField.PARENTS, prId));
            if (titl != null) filters.add(Filters.eq(SearchableField.TITLE, titl));
            if (mime != null) filters.add(Filters.eq(SearchableField.MIME_TYPE, mime));
            Query qry = new Query.Builder().addFilter(Filters.and(filters)).build();
            DriveApi.MetadataBufferResult result = Drive.DriveApi.query(mGAC, qry).await();
        ...
        if (buf != null) {
            if (mime != null) {
                ...
                if ((r1 != null) && (r1.getStatus().isSuccess())) try {
                    ...
                    GoogleDriveUtility.logError(e);
                }
            }

        } else {
            meta = new MetadataChangeSet.Builder().setTitle(titl).setMimeType(GoogleDriveUtility.MIME_FOLDER).build();
    ...
    static DriveId createTreeGDAA(String root, String titl, byte[] buf) {
        ...
        if (dId != null) {
            dId = findOrCreateFolder(dId, GoogleDriveUtility.titleToMonth(titl));
            if (dId != null) {
                return create(dId, titl + GoogleDriveUtility.JPEG_EXT, GoogleDriveUtility.MIME_JPEG, buf);
            }
        }
        return null;
    }
    ...
    private static DriveId findOrCreateFolder(DriveId prnt, String titl) {
        ArrayList<GF> gfs = search(prnt, titl, GoogleDriveUtility.MIME_FOLDER);
        if (gfs.size() > 0) {
    ...
    static ArrayList<GF> search(String prId, String titl, String mime) {
        ...
        try {
            qry = mGoogleService.files().list().setQ(qryClause)
        ...
        if (titl != null && isConnected()) {
            ...
            if (buf != null) {
                if (mime != null) {
                    ...
                    java.io.File jvFl;
                    try {
                        jvFl = GoogleDriveUtility.bytesToFile(buf,
                                java.io.File.createTempFile(GoogleDriveUtility.TMP_FILENAME, null, GoogleDriveUtility.sContext.getCacheDir()));
                        gFl = mGoogleService.files().insert(meta, new FileContent(mime, jvFl)).execute();
                    } catch (Exception e) {
                        GoogleDriveUtility.logError(e);
                    }
                ...
                meta.setMimeType(GoogleDriveUtility.MIME_FOLDER);
                try {
                    gFl = mGoogleService.files().insert(meta).execute();
                } catch (Exception e) {
                    GoogleDriveUtility.logError(e);
                }
                if (gFl != null && gFl.getId() != null) {
    ...
    static byte[] read(String rsId, int thmbSz) {
        ...
        if (isConnected() && rsId != null) try {
            File gFl = (thmbSz == 0) ?
                    mGoogleService.files().get(rsId).setFields("downloadUrl").execute() :
                    mGoogleService.files().get(rsId).setFields("thumbnailLink").execute();
            if (gFl != null) {
                String strUrl;
                if (thmbSz == 0)
                ...
                InputStream is = mGoogleService.getRequestFactory()
                        .buildGetRequest(new GenericUrl(strUrl)).execute().getContent();
                buf = GoogleDriveUtility.isToBytes(is);
            }
        } catch (Exception e) {
            GoogleDriveUtility.logError(e);
        }
