...
public class NotesAdapter extends RecyclerView.Adapter<NotesAdapter.NoteHolder> {
    ...
    public void onBindViewHolder(NoteHolder holder, int position) {
        ...
        holder.mId.setText(String.valueOf(selectedNote.getId()));
        holder.mTitle.setText(selectedNote.getTitle());

        // We have to deal with a note or a note list here, so
        // check for both types and process accordingly
        if(selectedNote.getType().equals(AppConstant.LIST)) {
            ...
            holder.mDescription.setText(selectedNote.getDescription());
        }
        holder.mDate.setText(String.format("%s %s", selectedNote.getDate(), selectedNote.getTime()));

        // Display an image, but only if we have one to display.
        if(selectedNote.getBitmap() != null) {
            holder.mImage.setImageBitmap(selectedNote.getBitmap());
            holder.mImage.setVisibility(View.VISIBLE);
        } else if(selectedNote.getImagePath().equals(AppConstant.NO_IMAGE)) {
            // No image, so hide.
